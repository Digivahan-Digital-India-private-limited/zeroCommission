...
function computeValues(s) {
  ...
  const totalLoanEmi = s.personalLoan + s.homeLoan + s.carLoan + s.twoWheeler + s.consumerLoan + s.goldLoan + s.otherEmi
  const ccObligation = Math.round(s.ccOutstanding * 0.05)
  ...
  const procFeeAmt = Math.round(eligibleLoan * s.procFee / 100)
  const gstOnProc = Math.round(procFeeAmt * 0.18)
  const insuranceAmt = Math.round(eligibleLoan * s.insurance / 100)
  const netDisbursed = eligibleLoan - procFeeAmt - gstOnProc - insuranceAmt - s.fixedMisc
  const btOutstanding = s.plOutstanding + s.ccDebtOutstanding
...
export default function EligibilityCalculatorPage() {
  ...
  const [foirPct, setFoir] = useState(50)
  ...
  const loadScenario = (sc) => {
    ...
    setProcFee(sc.procFee)
    setInsurance(sc.insurance)
    setFixedMisc(sc.fixedMisc)
  ...
  const resetCalculator = () => {
    setCustomerName(''); setIncome(100000); setFoir(50); setRoi(9.5)
                ...
                <div>
                  ...
                  <div className="space-y-2.5">
                    {[
                      ...
                      [`Processing Fee (${procFee}%)`, fmt(procFeeAmt), false],
                ...
                <div className="lg:col-span-7 space-y-6">
                  ...
                  <div className="bg-white rounded-2xl border border-[#e8edf5] p-5 md:p-7" style={{ boxShadow: '0 8px 30px rgba(15,24,87,0.05)' }}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                      ...
                      <div>
                        ...
                        <NumField value={foirPct} onChange={v => setFoir(Math.min(100, Math.max(0, v)))} />
                        <input type="range" min="10" max="70" step="1" value={foirPct} onChange={e => setFoir(Number(e.target.value))}
                          className="w-full h-1.5 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-[#0176C7] mt-3" />
                        <div className="flex justify-between text-[11px] text-slate-500 mt-1 font-bold"><span>10%</span><span>70%</span></div>
                      </div>
                    </div>
                  ...
                  <div className="bg-white rounded-2xl border border-[#e8edf5] p-5 md:p-7" style={{ boxShadow: '0 8px 30px rgba(15,24,87,0.05)' }}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                      <StepField label="Personal Loan EMI" icon={<Building2 size={11} />} value={personalLoan} onChange={setPersonalLoan} />
                      <StepField label="Home Loan EMI" icon={<Home size={11} />} value={homeLoan} onChange={setHomeLoan} />
                      <StepField label="Car Loan EMI" icon={<Car size={11} />} value={carLoan} onChange={setCarLoan} />
                      <StepField label="Two Wheeler Loan EMI" icon={<Bike size={11} />} value={twoWheeler} onChange={setTwoWheeler} />
                      <StepField label="Consumer Loan EMI" icon={<ShoppingCart size={11} />} value={consumerLoan} onChange={setConsumerLoan} />
                      <StepField label="Gold Loan EMI" icon={<Medal size={11} />} value={goldLoan} onChange={setGoldLoan} />
                      <StepField label="Other EMI" icon={<ListTodo size={11} />} value={otherEmi} onChange={setOtherEmi} />
                    </div>
                  ...
                  {/* Fee & Processing */}
                  <div className="bg-white rounded-2xl border border-[#e8edf5] p-5 md:p-7" style={{ boxShadow: '0 8px 30px rgba(15,24,87,0.05)' }}>
                    <SectionTitle icon={<SlidersHorizontal size={15} />} className="mb-5">
                      Fee & Processing Parameters <span className="text-slate-500 normal-case text-xs font-medium">(optional)</span>
                    </SectionTitle>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div>
                        <label className="text-xs font-black uppercase tracking-wider text-slate-750 mb-2 block flex items-center gap-1"><Percent size={12} className="text-[#0176C7]" /> Proc. Fee (%)</label>
                        <NumField value={procFee} onChange={setProcFee} />
                      </div>
                      <div>
                        <label className="text-xs font-black uppercase tracking-wider text-slate-750 mb-2 block flex items-center gap-1"><ShieldCheck size={12} className="text-[#0176C7]" /> Insurance (%)</label>
                        <NumField value={insurance} onChange={setInsurance} />
                      </div>
                      <div>
                        <label className="text-xs font-black uppercase tracking-wider text-slate-750 mb-2 block flex items-center gap-1"><IndianRupee size={12} className="text-[#0176C7]" /> Fixed Misc (₹)</label>
                        <NumField value={fixedMisc} onChange={setFixedMisc} />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white rounded-2xl border border-[#e8edf5] p-5" style={{ boxShadow: '0 8px 30px rgba(15,24,87,0.05)' }}>
                    ...
                    <div className="space-y-2.5 text-sm">
                      {[
                        ...
                        { label: `Processing Fee (${procFee}%):`, value: fmt(procFeeAmt) },
                        { label: 'GST on Proc Fee (18%):', value: fmt(gstOnProc) },
                        { label: `Insurance (${insurance}%):`, value: fmt(insuranceAmt) },
