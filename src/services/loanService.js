...
export const submitLoanApplication = async (formDataState) => {
  ...
  const response = await axios.post(buildZeroCommissionUrl('/zero/user/feature/apply_loan.php'), formData, {
    headers: {
...
export const sendApplicationOtp = async (identifier, isToken = false) => {
  ...
  const response = await axios.post(buildZeroCommissionUrl('/zero/user/feature/request_track_application.php'), formData, {
    headers: {
      Accept: 'application/json',
    },
  });
...
export const verifyApplicationOtp = async (email, otp) => {
  ...
  const response = await axios.post(buildZeroCommissionUrl('/zero/user/feature/track_application.php'), formData, {
    headers: {
      Accept: 'application/json',
    },
  });
  ...
  if (response.data.status === true) {
    ...
    const timeline = Array.isArray(rawTimeline)
      ? rawTimeline
      : [
        ...Object.entries(rawTimeline)
          .filter(([k]) => !isNaN(k))
          .map(([, v]) => v),
        ...(Array.isArray(rawTimeline.timeline) ? rawTimeline.timeline : [])
      ]
    ...
    return {
      ...
      data: {
        ...
        timeline: timeline,          // ✅ ab proper array milega
...
export const getAllApplications = async (params = {}) => {
  const response = await api.get('/applications', { params: { ...params, _t: Date.now() } });
  return response.data;
};
...
export const updateApplicationStatus = async (token, status) => {
  const response = await api.patch(`/applications/${token}/status`, { status });
  return response.data;
};
...
export const uploadDocuments = async (loanApplicationId, uploads, remarks = '', authToken = '') => {
  ...
  const response = await axios.post(buildZeroCommissionUrl('/zero/user/feature/upload_docs.php'), formData, {
    timeout: 60000,
    headers: {
  ...
  if (response.data.status === true) {
    return { success: true, document_id: response.data.document_id, message: response.data.message };
  } else {
...
export const adminLoginAPI = async (email) => {
  ...
  if (response.data.status === true || response.data.status === 'true' || response.data.status === 'success') {
    return { success: true, data: { email: email }, message: response.data.message };
  } else {
    throw new Error(response.data.message || 'Failed to send OTP');
  }
};
...
export const toggleApplicationView = async (token, viewed) => {
  const response = await api.patch(`/applications/${token}/view`, { viewed });
  return response.data;
};
...
export const exportApplicationsNextPay = async (ids) => {
  ...
  const response = await axios.post(
    buildNextPayUrl('/zero/admin/feature/export_application.php'),
    { ids },
    {
      headers: {
        'Authorization': token || '',
...
export const getApplicationStatusById = async (applicationId) => {
  ...
  const response = await axios.post(
    buildNextPayUrl('/zero/user/verify_otp_test.php'),
    formData,
    {
      headers: {
...
export const refreshApplicationStatus = async (authToken) => {
  const response = await axios.post(buildZeroCommissionUrl('/zero/user/feature/refresh_application.php'), {}, {
    headers: {
...
export const getFaqList = async () => {
  const response = await axios.get(buildZeroCommissionUrl('/zero/user/feature/faq/get_list.php'), {
...
export const submitContactForm = async (formDataState) => {
  ...
  const candidateUrls = [
    buildZeroCommissionUrl('/zero/user/feature//contact/contact.php'),
    buildZeroCommissionUrl('/zero/user/feature/contact/contact.php'),
  ];
...
export const createGuestTicket = async (formData) => {
  try {
    const response = await axios.post(
      buildZeroCommissionUrl("/zero/user/feature/consultation/create_guest_ticket.php"),
      formData,
      {
        headers: {
